  ...
  return (
    <Box className={classes.LoginBox} mb={5}>
      <Box className={classes.masBoxFlex}>
        <Typography variant="h6">{t('My Bundles')}</Typography>
        <Box display="flex">
          <Button
            variant="contained"
            size="large"
...
export const AddBundlePopup = ({ open, handleClose, callbackFun }) => {
  ...
  return (
    ...
    >
      <DialogTitle className={classes.dailogTitle} style={{ color:"#6345ED" }}>{t('Create a bundle')}</DialogTitle>
      <DialogContent>
        <Box>
          <Grid container spacing={2}>
            <Grid item xs={12} md={4}>
              <label> {t('Bundles Title')}</label>
            </Grid>
